...
inline bool possible(const float t, const float* const xs,
			const float d, const float* xs_end) {

		float left = -t;

		for (const float* it = xs; it != xs_end; ++it) {
			left = std::max(left + t, *it - d);
			const float right = *it + d;
...
int main()
{
	...
	for(int ti = 0; ti < t; ++ti) {
		...
		while (right - left > epsilon) {
			const float d = (left + right) / 2.0f;
			if (possible(t, x, d, x + n)) {
				right = d;
			} else {
				left = d;
			}
