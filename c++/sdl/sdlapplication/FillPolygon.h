...
// сравнивает две точки по их Y координате, получа€ на вход номера тих точек
class yComparator
{
public:
...
// провер€ет, заканчиваетс€ ли данное ребро в текущей строке
...
// находит абсциссу точки пересечени€ ребра с текущей строкой
class Intersector
{
public:
	const std::vector<ScreenPoint>& vertices;
...
// функци€ FillPolygon принимает вектор вершин а также размер экрана и функцию/функтор DrawPixel, принимающую x и y
template<class DrawPixelFunction>
void FillPolygon(const std::vector<ScreenPoint>& vertices, const ScreenPoint& ScreenSize, const DrawPixelFunction& DrawPixel)
{
	...
	// 3) создаЄм вектор номеров вершин, отсортированный по ординате
	std::vector<int> ys(n);
	for(int i = 0; i < n; ++i)
		ys[i] = i;
	...
	// 4) начинаем с минимального y, с пустым —ј– - списком активных ребер
	// 5) далее закрашиваем в цикле пока не дойдЄм до последней строки
	std::list<pii> SAR;
	...
	for(int currentY = minY; currentY <= maxY; )
	{
		...
		// 6) добавить в —ј– ребра, начинающиес€ с текущей строки
		// если они горизонтальные, то не добавл€ть а просто нарисовать
		while(nextVertice != ys.end() && vertices[*nextVertice][1] == currentY) // ищем вершины на этой строке
		{
			...
			// здесь нет нужды провер€ть на равенство, т.к. все пары точек с равным y уже прорисованы выше
		}

		// 7) удалить из —ј– ребра, заканчивающиес€ на этой строке
		...
		{
			assert(SAR.empty()); // –ебер тоже не должно быть
		...
		if(currentY < 0)
		{
			...
			// идЄм до ближайшей вершины или кра€ экрана
			currentY = std::min(vertices[*nextVertice][1], 0);
		}
		else
		{
			// в пределах экрана, нужно рисовать, пока не дойдЄм до ближайшей вершины или maxY включа€
			...
			--currentY; // возвращаемс€ к последней закрашенной строке

			// если это была последн€€ строка, то выход
			if(currentY == maxY)
				return;
