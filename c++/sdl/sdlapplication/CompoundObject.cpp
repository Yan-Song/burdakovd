...
{
	// раст€нуть все вложенные объекты относительно нашего центра (в их координатах это 0)
	for(ObjectList::iterator it = objects.begin(); it != objects.end(); ++it)
...
void CompoundObject3D::Draw(const SDLApplication *app, const Vector3D &base) const
{
	for(ObjectList::const_iterator it = objects.begin(); it != objects.end(); ++it)
		(*it)->Draw(app, base + Center);
}
