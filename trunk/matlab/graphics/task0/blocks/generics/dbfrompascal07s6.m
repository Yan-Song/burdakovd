function db = dbfrompascal07s6 (vocroot, varargin)
% DBFROMPASCAL07s  Construct DB from PASCAL VOC 2007 data
...
if ~strcmp(challenge, 'Segmentation')
    error('dbfrompascal07s6 does not know how to handle any other challenges\n');
end
...
if verbose
  fprintf('dbfrompascal07s6: which train : ''%s''\n', which_train) ;
  fprintf('dbfrompascal07s6: which test  : ''%s''\n', which_test) ;
  fprintf('dbfrompascal07s6: cat filter  : ''%s''\n', cat_filt) ;
  fprintf('dbfrompascal07s6: challenge   : ''%s''\n', challenge);
end
...
try
    ...
    cat_ids   = [2 7 12 15 19];
    % add a background category
    cat_names = cat_names(cat_ids);
    ...
    test_ids = train_ids(101:200);
    train_ids = train_ids(1:100);
    ...
    for s = 1:length(sets)
        ...
        for i = 1:length(ids)
            ...
            classseg = sprintf(VOCopts.seg.clsimgpath, ids{i});
            db.aspects(a).classsegReader = @() loadKeepingSomeClasses(classseg, [2 7 12 15 19]);
...
if verbose
    fprintf('dbfrompascal07s6: done.\n') ;
    fprintf('dbfrompascal07s6: %d aspects.\n',    length(db.aspects)) ;
    fprintf('dbfrompascal07s6: %d categories.\n', length(db.cat_names)) ;
end
