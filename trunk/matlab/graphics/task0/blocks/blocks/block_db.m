...
switch lower(bk.db_type)
  ...
  case 'pascal07official'
    db = dbfrompascal07official (bk.db_prefix, 'verbose', bk.verb) ;
    db.segs = db.aspects;
    for i = 1:length(db.segs)
      db.segs(i).seg = i;
    end
  ...
  case 'pascal07s'
    db = dbfrompascal07s (bk.db_prefix, 'verbose', bk.verb) ;
    db.segs = db.aspects;
    for i = 1:length(db.segs)
      db.segs(i).seg = i;
    end
  ...
  case 'pascal07s6'
    db = dbfrompascal07s6 (bk.db_prefix, 'verbose', bk.verb) ;
    db.segs = db.aspects;
    for i = 1:length(db.segs)
      db.segs(i).seg = i;
    end
    db.class_ids = [2 7 12 15 19 0]; % class_ids in seg_prefix
