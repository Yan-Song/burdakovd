...
void* checkedRealloc(void* old, size_t size)
{
	...
	if(p == NULL && size != 0)
	{
		mexPrintf("Failed to reallocate %d bytes of memory.\n", (int)size);
		*((char*)p) = 0; // force fail
	}
	return p;
}
...
void* checkedMalloc(size_t size)
{
	...
	if(p == NULL && size != 0)
	{
		mexPrintf("Failed to allocate %d bytes of memory.\n", (int)size);
		*((char*)p) = 0; // force fail
	}
	return p;
}

#define Malloc(type,n) (type *)checkedMalloc((n)*sizeof(type))
...
int parse_command_line(int nrhs, const mxArray *prhs[], char *model_file_name)
{
	...
	for(i=1;i<argc;i++)
	{
		...
		switch(argv[i-1][1])
		{
			...
			case 'w':
				++param.nr_weight;
				param.weight_label = (int *)checkedRealloc(param.weight_label,sizeof(int)*param.nr_weight);
				param.weight = (double *)checkedRealloc(param.weight,sizeof(double)*param.nr_weight);
