...
void RT::Camera::SetDirection(const RT::NormalizedVector3D& _direction)
{
	...
	// сначала находим направление, дл€ этого проецируем dy на экранную плоскость
	const Vector3D dsy_dest__ = dy - v * (dy * v);

	// если получилс€ ноль, то есть плоскость перпендикул€рна dy, то берЄм в качестве dsy_dest проекцию dz на плоскость
	// не очень хорошо, но на самом деле веро€тность этого мала
