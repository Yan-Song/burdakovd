    ...
    public class MemoryManager
    {
        ...
        private string[] GetRawAddonMessage(ComparableListOfDouble signature)
        {
            ...
            try
            {
                ...
                string text = BM.ReadUTF8String(pMessage + 0x14, Math.Min(limit, BM.ReadUInt(pMessage + 0x10)));
                string target = BM.ReadUTF8String(pTarget + 0x14, Math.Min(limit, BM.ReadUInt(pTarget + 0x10)));
                string DoNotRestart = BM.ReadUTF8String(pDoNotRestart + 0x14, Math.Min(limit, BM.ReadUInt(pDoNotRestart + 0x10)));
                string NeedPurchaseConfirmation =
                    BM.ReadUTF8String(pNeedPurchaseConfirmation + 0x14, Math.Min(limit, BM.ReadUInt(pNeedPurchaseConfirmation + 0x10)));
                string CurrentState = BM.ReadUTF8String(pCurrentState + 0x14, Math.Min(limit, BM.ReadUInt(pCurrentState + 0x10)));
                string NothingToDo = BM.ReadUTF8String(pNothingToDo + 0x14, Math.Min(limit, BM.ReadUInt(pNothingToDo + 0x10)));
                string GuildStats = BM.ReadUTF8String(pGuildStats + 0x14, Math.Min(limit, BM.ReadUInt(pGuildStats + 0x10)));
