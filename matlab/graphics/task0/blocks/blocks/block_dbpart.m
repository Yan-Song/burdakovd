...
switch lower(bk.db_type)
  ...
  case 'pascal07official'
      % разбиение происходит в block_db.m => dbfrompascal07official.m
    
  case 'pascal07s'
    trainsegs = find([db.segs.flag] == db.TRAIN);
    perm = (length(trainsegs)-49):length(trainsegs);
    valsegs = trainsegs(perm);
    for zz = 1:length(valsegs)
      db.segs(valsegs(zz)).flag = db.VALIDATION;
    end
    disp('flags:');
    disp([db.segs.flag]);
